...
__device__ __forceinline__ int64_t atomicSub(int64_t* address, int64_t val) {
    return ::atomicAdd((unsigned long long*) address, (unsigned long long) -val);
}

__device__ __forceinline__ uint64_t atomicCAS(uint64_t* address, uint64_t cmp, uint64_t val) {
    return ::atomicCAS((unsigned long long*) address, (unsigned long long) val, (unsigned long long) val);
}

__device__ __forceinline__ int64_t atomicCAS(int64_t* address, int64_t cmp, uint64_t val) {
    return ::atomicCAS((unsigned long long*) address, (unsigned long long) val, (unsigned long long) val);
}
