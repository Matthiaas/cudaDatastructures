...
std::function<void(ListGraph,uint32_t*, uint32_t)>>> listGraphAlgorithms = {
    {"bfs", {
      {1, list_graph_aglos::bfs<1>},
      {2, list_graph_aglos::bfs<2>},
      {4, list_graph_aglos::bfs<4>},
      {8, list_graph_aglos::bfs<8>},
      {16, list_graph_aglos::bfs<16>},
      {32, list_graph_aglos::bfs<32>},
    }},
    {"bfs_sharework", {
      {1, list_graph_aglos::bfs_sharework<1>},
      {2, list_graph_aglos::bfs_sharework<2>},
      {4, list_graph_aglos::bfs_sharework<4>},
      {8, list_graph_aglos::bfs_sharework<8>},
      {16, list_graph_aglos::bfs_sharework<16>},
      {32, list_graph_aglos::bfs_sharework<32>},
    }}
};


std::map<std::string, std::function<void(COOGraph,uint32_t*, uint32_t)>> cooGraphAlgorithms = {
    ...
    for (const auto& algo : params.graph_algos) {
      if (graph_type == "COO") {
        ...
        if (it == cooGraphAlgorithms.end()) {
          std::cerr << "Algorithm " << algo << " not found for COO" << std::endl;
          continue;
        }
        auto fun = it->second;
        COOGraph coo_graph = COOFromDefaultGraph(graph);
        COOGraph coo_graph_d = coo_graph.CopyToDevice();
        double ms = time_call(std::bind(fun, coo_graph_d, distances1, 0));
        std::cout << params.graph_name  << " COO " << algo << " 1 " << ms << std::endl;
        coo_graph.Free();
        coo_graph_d.Free();
      } else if (graph_type == "CSR") {
        auto it = listGraphAlgorithms.find(algo);
        if (it == listGraphAlgorithms.end()) {
          std::cerr << "Algorithm " << algo << " not found for CSR" << std::endl;
          continue;
        }
        for (const auto group_size : it->second) {
          ...
          double ms = time_call(std::bind(fun,list_graph_d, distances1, 0));
          std::cout << params.graph_name  << " CSR " << algo << " " << group_size.first << " " << ms << std::endl;
          list_graph.Free();
          list_graph_d.Free();
        }
      }
