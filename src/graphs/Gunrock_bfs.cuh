...
template <typename problem_t, operators::load_balance_t balancer>
struct enactor_t : gunrock::enactor_t<problem_t> {
  ...
  using frontier_t = typename enactor_t<problem_t, balancer>::frontier_t;
  ...
  void loop(gcuda::multi_context_t& context) override {
    ...
    operators::advance::execute<balancer>(G, E, search, context);
 ...
 */
template <typename graph_t, operators::load_balance_t balancer>
float run(graph_t& G,
  ...
  using enactor_type = enactor_t<problem_type, balancer>;
...
void gunrock_bfs(ListGraph list_graph, int* distances, int single_source) {
  ...
  vertex_t number_of_rows = list_graph.num_vertices;
  vertex_t number_of_columns = list_graph.num_vertices;
  edge_t number_of_nonzeros = list_graph.num_edges;

  thrust::device_vector<weight_t> Ax(number_of_nonzeros, 0);

  auto G = graph::build::from_csr<memory_space_t::device, graph::view_t::csr>(
      number_of_rows, number_of_columns, number_of_nonzeros,
      reinterpret_cast<int*>(list_graph.offsets),
      reinterpret_cast<int*>(list_graph.neighbors), Ax.data().get());
