...
__global__ void insert_kernel(const typename HashTable::key_type* keys,
                              const typename HashTable::value_type* values,
                              size_t count, HashTable call_on) {
  const size_t tid = helpers::global_thread_id();
  const size_t gid = tid / HashTable::cooperative_group_size;
  const auto group = cg::tiled_partition<HashTable::cooperative_group_size>(
...
template <typename HashTable>
__global__ void retrieve_kernel(const typename HashTable::key_type* keys,
