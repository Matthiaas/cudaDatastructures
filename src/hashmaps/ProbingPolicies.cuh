...
template <class KeyType, auto &HashFunction, bool UseBuckets,
          size_t CooperativeGroupSize = 16>
struct LinearProbingPolicy {
  static std::string GetName() { return "LinearProbingPolicy"; }
  ...
  __device__ size_t end() const noexcept { return ~0; }

 private:
  static constexpr size_t pos_inc = std::conditional_t<
      UseBuckets, std::integral_constant<size_t, 1>,
...
struct DoubleHashinglProbingPolicy {
  static std::string GetName() { return "DoubleHashinglProbingPolicy"; }
  ...
  __device__ size_t begin(const KeyType key) noexcept {
    ...
    if constexpr (UseBuckets) {
      ...
      base_ =
          (HashFunction(key + 1) % (bucket_num_ / CooperativeGroupSize - 1) +
           1) *
